...
def make_scene_from_card(card: np.ndarray, rotate_deg: float = 7.0) -> np.ndarray:
    """Place a (possibly damaged) card on a contrasting background and rotate."""
    ch, cw = card.shape[:2]
...
def make_scene(
    ...
    return make_scene_from_card(make_card(left, right, top, bottom), rotate_deg)
